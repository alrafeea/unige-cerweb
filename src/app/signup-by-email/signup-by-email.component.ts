...
export class SignUpByEmailComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.signUpForm = this.fb.group({
      ...
      email: new FormControl('', [Validators.required, Validators.pattern('^[a-z_A-Z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,4}$')]),
  ...
  async signUp() {
    ...
    const dateValid =  this.isGoodDate($('#date-input').val());
    if (!dateValid) {
      this.validDateFormat = false;
      return;
    }
    const dateAgeValid = this.getValidAge($('#date-input').val());
    if (!dateAgeValid) {
      this.validDateAge = false;
      return;
    }

    const auth = firebase.auth();
    try {
      const { user } = await auth.createUserWithEmailAndPassword(this.email.value, this.password.value);
      // TODO: return this code back when sendEmailVerification needed.
      // user.sendEmailVerification().then(res => {
      //   console.log('res',res);
      //   // show an email sent message and navigate back to login
      // }).catch(err => {
